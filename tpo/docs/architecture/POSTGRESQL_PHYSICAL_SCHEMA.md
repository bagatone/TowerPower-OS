...
- `consegna_id bigint NOT NULL` FK;
- `ordine_id bigint NOT NULL` FK;
- `posizione integer NOT NULL` > 0.

PK composta (`consegna_id`, `ordine_id`); UNIQUE (`consegna_id`, `posizione`). Il CLIENTE dei due aggregati deve coincidere, verificato transazionalmente.

Indici: PK; indice su `ordine_id` per calcolo evasione.

Delete: CASCADE con una consegna non ancora avvenuta; RESTRICT dopo CONSEGNATA.
...
Vincoli: UNIQUE (`consegna_id`, `posizione`). Per consegne ordinarie, varietà e quantità devono essere riconducibili alle righe degli ORDINI collegati; il controllo aggregato avviene nella transazione di consegna.

Indici: FK consegna; FK varietà.

Delete: CASCADE con la testata soltanto prima dell’evento; immutabile dopo CONSEGNATA.
...
Foreign key composta (`varieta_id`, `unita_misura`) → STOCK. Per origine RACCOLTA o CONSEGNA è obbligatoria la rispettiva FK e vietata l’altra; origini SCARTO, RETTIFICA o future usano tipo e riferimento senza creare una FK polimorfica falsa.

Indici: UK public ID; (`varieta_id`, `data_movimento`); tipo; origine; raccolta; consegna; RUN.
...
La transizione a CONSEGNATA, i collegamenti definitivi agli ORDINI, gli eventuali MOVIMENTI_MAGAZZINO e l’aggiornamento STOCK avvengono in una singola transazione orchestrata. Una CONSEGNA ANNULLATA non produce MOVIMENTI.
