...
| `public_id` | `text` | NOT NULL | nessuno | AllocazioneId immutabile |
| `tipo` | `allocation_type` | NOT NULL | nessuno | immutabile |
| `riga_piano_semina_id` | `bigint` | NOT NULL | nessuno | destinazione immutabile |
| `quantita` | `numeric(20,6)` | NOT NULL | nessuno | immutabile dopo INSERT |
| `unita_misura` | `unit_of_measure` | NOT NULL | nessuno | immutabile |
| `stato` | `planning_allocation_state` | NOT NULL | `ATTIVA` | lifecycle mutabile CAS |
| `created_at` | `timestamptz` | NOT NULL | nessuno | immutabile |
| `created_by` | `text` | NOT NULL | nessuno | immutabile |
| `updated_at` | `timestamptz` | NOT NULL | nessuno | mutabile con lifecycle |
| `updated_by` | `text` | NOT NULL | nessuno | mutabile con lifecycle |
| `version` | `bigint` | NOT NULL | `0` | optimistic concurrency |

PK `id`; FK `riga_piano_semina_id` → `tpo.righe_piano_semina(id)` ON UPDATE
RESTRICT ON DELETE RESTRICT; UNIQUE `public_id`; CHECK formato
`^ALL-[0-9]{6,}$`, quantità `>0`, actor non vuoti, `version>=0`; indici
(`riga_piano_semina_id`,`stato`) e (`tipo`,`stato`). `allocation_type` è enum
`DOMANDA|STOCK|PRODUZIONE_IN_CORSO|RACCOLTA`. Lifecycle esclusivo
ATTIVA→CONSUMATA|RILASCIATA|SOSTITUITA|INVALIDA; terminali non riattivabili.
Nessun hard delete. Writer: Production Planning allocation writer.

Ogni parent possiede esattamente una child dello stesso tipo. Un constraint
trigger differibile, puramente strutturale, verifica a fine transazione presenza
e corrispondenza della child; non applica business logic.

### 7.1 `tpo.allocazioni_domanda`

Aggiunge `allocation_id bigint NOT NULL` PK/FK → `tpo.allocazioni(id)` ON UPDATE
RESTRICT ON DELETE RESTRICT e `riga_ordine_id bigint NOT NULL` FK →
`tpo.righe_ordine(id)` RESTRICT. Nessun'altra colonna, version o audit. UNIQUE
(`riga_ordine_id`,`allocation_id`); indice `riga_ordine_id`. Parent tipo DOMANDA.
Append-only; writer allocation. Il writer blocca ordine/riga e
garantisce copertura entro residuo.
...
| `state` | `planning_allocation_state` | NOT NULL | NONE | lifecycle |
| `created_at` | `timestamptz` | NOT NULL | NONE | istante applicativo |
| `created_by` | `text` | NOT NULL | NONE | actor applicativo |
| `version` | `bigint` | NOT NULL | `0` | token CAS |

**PRIMARY KEY:** `pk_allocazioni` (`id`). **FOREIGN KEYS:** nessuna. **UNIQUE
CONSTRAINTS:** `uq_allocazioni_public_id` (`public_id`). **CHECK CONSTRAINTS:**
`ck_allocazioni_public_id` (formato `ALL-*`), `ck_allocazioni_quantity`
(`quantity > 0`), `ck_allocazioni_version` (`version >= 0`),
`ck_allocazioni_created_by` (`btrim(created_by) <> ''`). **OTHER STRUCTURAL
...
e che la sua tabella coincida con `allocation_type`. **INDEXES:**
`ix_allocazioni_type_state` (`allocation_type`,`state`). **MUTABILITY:** soltanto
transizioni lifecycle e `version`. **OPTIMISTIC CONCURRENCY:** UPDATE con
`WHERE id=:id AND version=:expected_version`, incremento atomico di `version`.
**DELETE POLICY:** RESTRICT dopo creazione. **WRITER AUTHORITY:** allocation
writer del Planning Commit Repository.
...
**PRIMARY KEY:** `pk_allocazioni_domanda` (`allocation_id`). **FOREIGN KEYS:**
`fk_allocazioni_domanda_allocation`: `allocation_id` → `tpo.allocazioni(id)` ON
UPDATE RESTRICT ON DELETE RESTRICT; `fk_allocazioni_domanda_riga_ordine`:
`riga_ordine_id` → `tpo.righe_ordine(id)` ON UPDATE RESTRICT ON DELETE RESTRICT.
