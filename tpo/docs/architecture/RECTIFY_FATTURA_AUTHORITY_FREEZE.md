...
**Status:** OWNER-APPROVED ARCHITECTURE FREEZE (2026-09-05, "ok" a fronte delle
Owner Decisions D8-D10 raccolte nella stessa sessione).
...
- `varieta_id` della riga rettificativa è anch'esso copiato dalla riga
  originale (stessa varietà, mai diversa — non si "cambia prodotto" con
  una rettifica).
...
- Trigger `fn_righe_fattura_rettifica_coerente` (constraint trigger,
  ...
  valorizzato), verifica che `varieta_id` coincida con l'originale, e
  che la `FATTURA` rettificativa referenzi (via `rettifica_di`) proprio
  la fattura a cui appartiene la riga originale.
...
- `uq_righe_fattura_rettifica_riga_fattura` (UNIQUE su
  `rettifica_riga_fattura_id`): una riga originale è correggibile una sola
  volta.
...
RectifyFattura(
    ...
    data_emissione: date,                     # come EmitFattura, caller-supplied
...
del chiamante, §4). `numero_fattura`, `scadenza`,
`importo_netto`/`importo_igic`, `totale_netto`/`totale_igic`/`totale` sono
writer-owned (stesso schema di classificazione campi di
`FATTURA_AUTHORITY_FREEZE.md` §11 — `data_emissione` è caller-supplied
esattamente come per `EmitFattura`, per coerenza con quel comando
gemello).
...
(`rettifica_di`, `data_emissione`, l'insieme esatto ordinato di
(`posizione_originale`, `quantita`)), stessa transazione singola per
reservation + numerazione + insert + audit. Audit: `tpo.audit_eventi`,
`entity_type='FATTURA'`, `entity_public_id` = il nuovo `numero_fattura`
della rettifica, `operation='CORRECTION'`.
...
`src/tpo_core/application/fattura_rettifica/{__init__,models,ports,service,errors}.py`.
...
Bootstrap: `src/tpo_core/bootstrap/fattura_rettifica.py`. CLI: nuovo
sottocomando `fattura rettifica` in `cli/fattura.py` e `cli/main.py`
(`--rettifica-di`, `--riga POSIZIONE:QUANTITA` ripetibile, `--data-emissione`,
`--actor`, `--reason`, `--correlation-id`, `--idempotency-key`, `--confirm`).
Migrazione: `20260905_0029_fattura_rettifica.py`. Test: dominio
(`tests/application/test_fattura_rettifica.py`), CLI
(`tests/cli/test_fattura_rettifica_cli.py`), migrazione
(`tests/infrastructure/postgresql/test_fattura_rettifica_migration.py`),
writer/integrazione PostgreSQL reale
