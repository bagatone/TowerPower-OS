...
try:
    run(
        ...
        "WHERE denominazione ILIKE %s OR codice_tracciabilita='HIN'",
        ("%hinojo%",),
    )
    run(
        "LOTTO SEME LSE-000005",
        ...
        "WHERE ls.public_id='LSE-000005'",
    )
    run(
        "SEMENTE_IMPIEGHI collegati a Hinojo",
        ...
        "WHERE v.codice_tracciabilita='HIN' OR v.denominazione ILIKE %s",
        ("%hinojo%",),
    )
    run(
        "PROTOCOLLO_VERSIONI collegati a Hinojo (ogni versione, non solo corrente)",
    ...
    run(
        "SEMINE gia' esistenti sul lotto LSE-000005",
        "SELECT public_id, codice_tracciabilita, stato, data_avvio, quantita_seme, unita_misura "
        "FROM tpo.semine WHERE lotto_seme_id=(SELECT id FROM tpo.lotti_seme WHERE public_id='LSE-000005')",
    )
finally:
    conn.close()
